FactoryBot.define do
  ...
  # - Travis 3/2020
  # These factories are derived from Miras's collection factories and the following comments are from epigaea.
  # I have removed some factories and traits that Trove doesn't use.
  ...
  # Won't work because it doesn't have a CollectionType. Use either :personal_collection or :course_collection.
  factory :collection do
    ...
    after(:build) do |collection, evaluator|
      ...
      collection.member_of_collections = [evaluator.parent] if(evaluator.parent)

      CollectionFactoryHelper.process_with_permission_template(collection, evaluator)
      CollectionFactoryHelper.process_with_solr_document(collection, evaluator)
      CollectionFactoryHelper.process_with_nesting_attributes(collection, evaluator)
    end

    # Personal Collections
    factory :personal_collection do
      sequence(:title) { |n| ["Personal Collection - #{n}"] }
    ...
    factory :course_collection do
      sequence(:title) { |n| ["Course Collection - #{n}"] }

      after(:build) do |collection, evaluator|
        # CollectionTypes are instantiated in rails_helper.
        collection.collection_type_gid = Hyrax::CollectionType.where(title: 'Course Collection').first.gid
      end
    end
  end
  ...
  class CollectionFactoryHelper
    ...
    def self.process_with_solr_document(collection, evaluator)
      ...
      ActiveFedora::SolrService.add(solr_document_with_permissions(collection, evaluator), commit: true) #This'll become Hyrax::SolrService after upgrade
    end
